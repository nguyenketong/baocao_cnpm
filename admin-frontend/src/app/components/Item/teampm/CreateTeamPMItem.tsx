...
const CreateTeamPMItem: React.FC<CreateTeamItemProps> = ({ 
  onClose, 
  currentTime = "2025-03-02 12:06:53",
  currentUser = "HMK1510" 
}) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const employeeRes = await apiService.fetchEmployees();
        ...
        if (currentEmployee?.designation_id?.designationName === 'IT Project Manager') {
          const pmProjects = await apiService.fetchProjectsForPM(currentEmployee._id);
          setProjects(pmProjects.data);
        } else {
          const projectRes = await apiService.fetchProjects();
...
export default CreateTeamPMItem;