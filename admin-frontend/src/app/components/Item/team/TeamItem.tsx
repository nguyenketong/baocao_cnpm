...
import ViewTeamMembers from './ViewTeamMembers'; // Import the new component
...
const TeamItem: React.FC<TeamItemProps> = ({ team }) => {
  ...
  useEffect(() => {
    ...
    console.log(`[TeamItem] Đăng ký observer cho team ${team._id}`);
    teamObserver.subscribe(updateTeam);
  
    return () => {
      console.log(`[TeamItem] Hủy đăng ký observer cho team ${team._id}`);
      teamObserver.unsubscribe(updateTeam);
    };
  }, [team._id]);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      teamObserver.notify({ _id: team._id, deleted: true }); //Notify subscribers
    } catch (error) {
      ...
      <div className="flex flex-col w-full">
        ...
        <div className="text-xs bg-slate-400 p-2 font-bold inline-block text-center rounded-sm w-fit px-2 py-1 border-2 border-slate-450">
          <span className="text-black">
            {teamState.projectid?.projectName || 'No Project Assigned'}
          </span>
        </div>
        ...
        <div className="text-sm">
          <div className="mt-3 pt-3 border-t border-gray-200">
            ...
            <span className="text-gray-600">
              {teamState.teamLead?.employeeName || 'No Team Lead Assigned'}
            </span>
          </div>
        </div>
